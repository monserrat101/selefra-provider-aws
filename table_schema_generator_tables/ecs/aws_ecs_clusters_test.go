...
func buildEcsClusterMock(t *testing.T, ctrl *gomock.Controller) aws_client.AwsServices {
	...
	ecsListOutput := &ecs.ListClustersOutput{
		...
		ClusterArns: []string{constants.RandomClusteArn},
	...
	servicesList := ecs.ListServicesOutput{
		...
		ServiceArns: []string{constants.Test},
	}
	...
	instancesList := ecs.ListContainerInstancesOutput{
		...
		ContainerInstanceArns: []string{constants.Test},
	}
	...
	m.EXPECT().GetTaskProtection(gomock.Any(), gomock.Any(), gomock.Any()).AnyTimes().Return(&protection, nil)
