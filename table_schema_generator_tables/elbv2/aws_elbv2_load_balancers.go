...
func (x *TableAwsElbv2LoadBalancersGenerator) GetColumns() []*schema.Column {
	return []*schema.Column{
		table_schema_generator.NewColumnBuilder().ColumnName("scheme").ColumnType(schema.ColumnTypeString).
			Extractor(column_value_extractor.StructSelector("Scheme")).Build(),
		...
		table_schema_generator.NewColumnBuilder().ColumnName("canonical_hosted_zone_id").ColumnType(schema.ColumnTypeString).
			Extractor(column_value_extractor.StructSelector("CanonicalHostedZoneId")).Build(),
